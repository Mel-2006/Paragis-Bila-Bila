...
export async function getDashboardStats() {
  ...
  // Return raw lists too so callers don't need to re-fetch
  return { totalProducts, totalStock, lowStock, inventoryValue, todaySales, totalSales: sales.length, products, sales };
}
...
export async function getProfile() {
  try {
    ...
    console.warn('getProfile failed:', err.message);
    const user = auth.currentUser;
...
export async function updateProfile(data) {
  ...
  try {
    return await setDoc(doc(db, 'users', user.uid), data, { merge: true });
  } catch (err) {
